...
RUNBAN_ERRORS = RBAN_ERRORS
RKICK_ERRORS = RBAN_ERRORS
RMUTE_ERRORS = RBAN_ERRORS
RUNMUTE_ERRORS = RBAN_ERRORS

def run_async(func):
    # Dummy decorator for compatibility if using sync handlers
    return func
...
def runban(bot: Bot, update: Update, args: List[str]):
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.warning(update)
            LOGGER.exception("ERROR unbanning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id, excp.message)
...
def rkick(bot: Bot, update: Update, args: List[str]):
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.warning(update)
            LOGGER.exception("ERROR kicking user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id, excp.message)
...
def rmute(bot: Bot, update: Update, args: List[str]):
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.warning(update)
            LOGGER.exception("ERROR muting user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id, excp.message)
...
def runmute(bot: Bot, update: Update, args: List[str]):
    ...
    if is_user_in_chat(chat, user_id):
        if getattr(member, "can_send_messages", True) and \
           getattr(member, "can_send_media_messages", True) and \
           getattr(member, "can_send_other_messages", True) and \
           getattr(member, "can_add_web_page_previews", True):
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.warning(update)
            LOGGER.exception("ERROR unmuting user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id, excp.message)
